...
:::note
The first time you will run SocNetV, macOS may tell you that it cannot verify the software developer/publisher. That's because we do not always sign our released packages with apple developer signing certificates (which are not free by the way). 
...
Then, make the .appimage file executable and double-click on it to run SocNetV. That's it. :) Note that [AppImages require libfuse2](https://github.com/socnetv/app/issues/166) to work. On latest Ubuntu LTS releases, you need to install it beforehand: 
