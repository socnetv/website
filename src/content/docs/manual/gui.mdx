...
- **Edit**: Tools for modifying the network:
  ...
  - **Filter** submenu: non-destructive exploration filters (ego network, focus on selection, attribute-based, centrality, edge weight) and restore actions.
  - Toggle the **Data Table dock** (`Ctrl+T`) for tabular node/edge editing, export, and import.
...
- **Focus on Selection**: hide all nodes not in the current selection (`Ctrl+X, Ctrl+S`).
...
```
[ Nodes: ego network × ]  [ Nodes: type = investor × ]  [Clear all]
```

- The **×** on the most recently applied chip removes that filter and pops one entry from the stack.
- Chips for earlier filters show a disabled × with a tooltip explaining the order constraint — filters must be removed in reverse order of application.
